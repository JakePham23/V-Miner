...
$$
(\Sigma , \Sigma_ {i}) = \left(- \partial_ {k} \Pi_ {k} ^ {V}, \varepsilon^ {i j k} \partial_ {j} \left(\Pi_ {k} ^ {B} - m V _ {k}\right) - \mu^ {2} B _ {i}\right). \tag {14}
$$
...
All other constraints are first class and no further constraints need to be imposed for consistency. There are consequently five first class constraints $(\varPhi^{U},\varPhi_{k}^{A}$ and $\Sigma$ ) and six second class constraints $(\Sigma_{i}$ and $T_{k})$ . The constraints $\varPhi^U$ and $\Sigma$ correspond to the usual gauge transformations $\delta W_0 = \partial_0\Omega$ $\delta W_{i} = \partial_{i}\Omega$ associated with a gauge field $W_{\mu}$ , while $\varPhi_{k}^{A}$ is associated with the fact that in (12) $A_{k}$ acts merely as a Lagrange multiplier (i.e., it is not dynamical) and hence its value is completely arbitrary. Suitable gauge conditions associated with the first class constraints are

$$
\left(\gamma^ {U}, \gamma_ {k} ^ {A}, \gamma^ {V}\right) = \left(U, A _ {k}, \partial_ {k} V _ {k}\right) = 0. \tag {17}
$$
